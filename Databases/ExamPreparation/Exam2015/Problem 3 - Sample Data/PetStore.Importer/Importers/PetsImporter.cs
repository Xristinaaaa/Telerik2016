using PetStore.Data;
using System;
using System.IO;
using System.Linq;

namespace PetStore.Importer.Importers
{
    public class PetsImporter : IImporter
    {
        private const int PetsToAdd = 5000;

        public Action<PetStoreEntities, TextWriter> Import
        {
            get
            {
                return (db, tr) =>
                {
                    db.Configuration.AutoDetectChangesEnabled = false;
                    db.Configuration.ValidateOnSaveEnabled = false;

                    var petsIds = db.Species.Select(s => s.Id).ToList();
                    var colorIds = db.Colors.Select(c => c.Id).ToList();

                    var addedPets = 0;

                    foreach (var petsId in petsIds)
                    {
                        var petsPerSpecies = RandomGenerator.RandomNumber(25, 75);

                        if (addedPets + 75 >= PetsToAdd)
                        {
                            petsPerSpecies = PetsToAdd - addedPets;
                        }

                        for (int i = 0; i < petsPerSpecies; i++)
                        {
                            this.AddSpecies(db, petsId, colorIds[RandomGenerator.RandomNumber(0, colorIds.Count - 1)]);

                            addedPets++;
                        }

                        tr.Write(".");
                        db.SaveChanges();
                        db = new PetStoreEntities();

                        db.Configuration.AutoDetectChangesEnabled = false;
                        db.Configuration.ValidateOnSaveEnabled = false;
                    }

                    if (addedPets < PetsToAdd)
                    {
                        var leftPets = PetsToAdd - addedPets;
                        for (int i = 0; i < leftPets; i++)
                        {
                            this.AddSpecies(
                                db,
                                petsIds[RandomGenerator.RandomNumber(0, petsIds.Count - 1)],
                                colorIds[RandomGenerator.RandomNumber(0, colorIds.Count - 1)]);

                            addedPets++;
                        }
                    }

                    db.SaveChanges();
                    db.Configuration.AutoDetectChangesEnabled = true;
                    db.Configuration.ValidateOnSaveEnabled = true;
                };
            }
        }
        private void AddSpecies(PetStoreEntities db, int speciesId, int colorId)
        {
            db.Pets.Add(new Pet
            {
                BirthDateTime = RandomGenerator.RandomDateTime(),
                Breed = RandomGenerator.RandomString(5, 30),
                ColorId = colorId,
                Price = RandomGenerator.RandomNumber(5, 2500),
                SpeciesId = speciesId
            });
        }
        public string Message
        {
            get
            {
                return "Importing pets";
            }
        }

        public int Order
        {
            get
            {
                return 3;
            }
        }
    }
}
