namespace ConsoleWebServer.Framework.Actions
{
    ...
    {
        private const string NotFoundHttp = "Expected method with signature IActionResult {0}(string) in class {1}Controller";
        public IActionResult InvokeAction(Controller c, ActionDescriptor ad)
        {
            ...
            */
            ...
            var methodWithIntParameter = c.GetType().GetMethods().FirstOrDefault(x => x.Name.ToLower() == ad.ActionName.ToLower()
            ...
            {
                throw new HttpNotFound(string.Format(NotFoundHttp, ad.ActionName, ad.ControllerName));
            }
